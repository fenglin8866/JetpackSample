...
package com.program.example.persistence.ui;
...
import com.program.databinding.ListFragmentBinding;
import com.program.example.persistence.db.entity.ProductEntity;
import com.program.example.persistence.viewmodel.ProductListViewModel;
...
public class ProductListFragment extends Fragment {
    ...
    private final ProductClickCallback mProductClickCallback = product -> {
        if (getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.STARTED)) {
            ((ProductActivity) requireActivity()).show(product);
        }
    };
}
