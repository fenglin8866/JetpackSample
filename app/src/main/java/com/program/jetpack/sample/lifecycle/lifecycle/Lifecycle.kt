package com.program.jetpack.sample.lifecycle.lifecycle
...
abstract class Lifecycle {
    ...
    @MainThread
    abstract fun addObserve(observer: LifecycleObserver)

    @MainThread
    abstract fun removeObserve(observer: LifecycleObserver)
...
abstract class LifecycleCoroutineScope internal constructor() : CoroutineScope {
     ...
     */
    @Deprecated(
        message = "launchWhenCreated 已弃用，因为它在某些情况下会导致资源浪费。" +
                "替换为 suspending repeatOnLifecycle，以便在 Lifecycle 状态至少为 Lifecycle.State.CREATED 时运行该块。",
        replaceWith =
            ReplaceWith("launch { lifecycle.whenCreated(block) }", "kotlinx.coroutines.launch")
    )
